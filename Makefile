...
# ODK Version: v1.3.1
# ----------------------------------------
# IMPORTANT: DO NOT EDIT THIS FILE. To override default make goals, use bao.Makefile instead
...
$(REPORTDIR)/$(SRC)-obo-report.tsv: $(SRCMERGED) | $(REPORTDIR)
	$(ROBOT) report -i $< $(REPORT_LABEL) $(REPORT_PROFILE_OPTS) --fail-on $(REPORT_FAIL_ON) --base-iri $(URIBASE)/BAO_ --base-iri $(URIBASE)/bao --print 5 -o $@
...
$(ONTOLOGYTERMS): $(SRC) $(OTHER_SRC)
	touch $(ONTOLOGYTERMS) && \
	$(ROBOT) query --use-graphs true -f csv -i $< --query ../sparql/bao_terms.sparql $@
...
validate_idranges:
	amm $(SCRIPTSDIR)/validate_id_ranges.sc bao-idranges.owl
...
* update_repo:		Update the ODK repository setup using the config file bao-odk.yaml
...
* normalize_src:	Load and safe your bao-edit file after you to make sure its serialised correctly
...
* custom_reports:	Generate all custom sparql reports you have configured in your bao-odk.yaml file.
...
* validate_profile_%:	Run an OWL2 DL profile validation check, for example validate_profile_bao-edit.owl.
